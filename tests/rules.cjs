...
test('alcohol trade requires geography','S.cash=450;S.ap=12;selected=S.loc="pub";let ap=S.ap;action("booze");if(S.ap!==ap||S.booze!==0)throw Error("pub buy");selected=S.loc="station";action("booze");if(S.booze!==1||S.cash!==385)throw Error("station buy");S.market.pub=130;selected=S.loc="pub";action("sell");if(S.booze!==0||S.cash!==515||S.score!==1)throw Error("pub sell")');
test('passport is persistent useful asset and not repurchased','S.cash=1200;S.heat=80;S.ap=12;action("passport");if(!S.passport||S.cash!==700||S.heat!==45)throw Error("passport");let ap=S.ap;action("passport");if(S.ap!==ap||S.cash!==700)throw Error("duplicate passport")');
