/* Rules revision 3. All encounter state is serializable; legacy saves are retained. */
...
function upgrade(s){
 ...
 s.heat=clamp(s.heat,0,100);s.score=clamp(s.score,0,100);s.m=clamp(s.m,1,12);s.y=clamp(s.y,1925,1928);
 ...
 s.ap=clamp(s.ap??12,0,12);s.market=s.market||{price:125,supply:8,demand:8};s.used=s.used||{};s.revision=3;
 return s;
}
try{if(S.revision!==3)localStorage.setItem('uw1931-before-v3',JSON.stringify(S));S=upgrade(S)}catch(e){S=upgrade(fresh())}selected=S.loc;
...
function checkEnd(){if(S.over)return;if(S.score>=100&&S.mayorDone&&S.transportDone)S.over='win';else if(S.y>=1928||S.debt>15000)S.over='lose'}
